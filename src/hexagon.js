...
import roundPathCorners from './roundcorners.js' 
...
function Hexagon(props) {
  ...
  const polyStyle = defaults({
    fill: props.backgroundImage ? `url(#${bgId})` : '#f5f5f5',
    stroke: '#000000',
  ...
  // Compute bounds by reducing points
  ...
  let strpoints = points.map(point => point.join(',')).join(' ');
  ...
  const polygon = (
    <polygon
      ...
      style={polyStyle}
      points={ roundPathCorners(strpoints, 0.5, true) }
    />
  )
